// http://localhost:3001/?tkn=OTA2NTQ3MTcwMDUzNTY1MQ==&invn=SlMvMTg0LzIwLTI1&evn=c2FsZQ==&pnm=RXhwb3J0IEludm9pY2UgUw==&up=aHR0cDovL3plbi9qby9hcGktbGliL0FwcC9TYWxlQmlsbF9Kc29u&ctv=NzE=&ifid=ExportInvoiceA&pid=undefined
import React, { useEffect, useState } from "react";
import "../../assets/css/prints/exportinvoices.css";
import {
  apiCall,
  checkMsg,
  fixedValues,
...
const ExportInvoiceS = ({ token, invoiceNo, printName, urls, evn, ApiVer }) => {
  ...
  const [metalGoldPurityWiseTotal, setMetalGoldPurityWiseTotal] =
    useState(null);

  const [metalWithoutGoldPurityWise, setMetalWithoutGoldPurityWise] = useState(
    []
  );
  const [metalWithoutGoldPurityWiseTotal, setMetalWithoutGoldPurityWiseTotal] =
    useState(null);
  ...
  const loadData = (data) => {
    ...
    datas?.resultArray?.forEach((e) => {
      ...
      let obj = cloneDeep(e);
      let find_record = grpArr?.findIndex(
        (el) =>
          el?.MetalTypePurity === obj?.MetalTypePurity &&
          el?.Categoryname === obj?.Categoryname
      );
      if (find_record === -1) {
        grpArr.push(obj);
      } else {
    ...
    met_arr_gold?.forEach((a) => {
      let obj = cloneDeep(a);
      let findrec = met_arr.findIndex(
        (e) => e?.MetalTypePurity === obj?.MetalTypePurity
      );
      if (findrec === -1) {
        met_arr.push(obj);
      } else {
    ...
    met_arr2_withoutgold?.forEach((a) => {
      let obj = cloneDeep(a);
      let findrec = met_arr2.findIndex(
        (e) => e?.MetalTypePurity === obj?.MetalTypePurity
      );
      if (findrec === -1) {
        met_arr2.push(obj);
    ...
    datas?.json2?.forEach((a) => {
      if (a?.MasterManagement_DiamondStoneTypeid === 5) {
        ...
        let obj = cloneDeep(a);
        let findrec = fin_arr?.findIndex(
          (el) => el?.FindingAccessories === obj?.FindingAccessories
        );
        if (findrec === -1) {
          fin_arr.push(obj);
        } else {
    ...
    data?.BillPrint_Json1.forEach((e, i) => {
      ...
      let findIndex = NewArr.findIndex(
        (ele, ind) => ele?.designno === e?.designno
      );
      gross += e?.grosswt;
      ...
      {loader ? (
        ...
        <>
          {msg === "" ? (
            <div className="export_print_a_container ">
              ...
              <div className="first_page_eia">
                <div className="d-flex border border-black">
                  <div className="p-2 col-6 border-end border-black">
                    ...
                    <p className="headline_fs_eia fw-bold py-1">
                    ...
                    <p className="fw-semibold">
                      Telephone No :&nbsp;{result?.header?.CompanyTellNo}
                    </p>
                    <p className="fw-semibold">
                      Email Id :&nbsp;{result?.header?.CompanyEmail}
                    </p>
                  </div>
                  <div className="col-6">
                    <div className="d-flex border-bottom border-black">
                      ...
                      <div className="col-5 d-flex flex-column justify-content-between align-items-center p-2">
                        <p className="text-uppercase fw-semibold w-100 text-start">
                          Exporter's Ref.
                        </p>
                ...
                {/* <div className={` w-100  h-100`}>
                  <div className=" border border-top-0 border-black p-1 text-start">
                    <p className="fw-normal">
                      ...
                      &nbsp;&nbsp;&nbsp; No & KIND OF PKGS : 1 (ONE) TIN BOX
                    </p>
                  </div>
                  <div className=" border border-top-0 border-black p-1 text-start">
                    <p className="fw-normal">
                      <span className="fw-bold">AS ADD </span>{" "}
                      {metalValue?.QualityName} Branded {metalValue?.ShapeName}{" "}
                      & {silverValue?.QualityName} {silverValue?.ShapeName}{" "}
                      Plain Jewellery
                    </p>
                  </div>
                </div>
                <div>
                  <div className="d-flex fw-bold border border-black mt-1">
                    ...
                    <div className="col_4_eia p-1">
                      Dia Wt <br /> (Cts)
                    </div>
                    <div className="col_5_eia p-1">
                      CS Wt <br /> (Cts)
                    </div>
                    <div className="col_6_eia border-end border-black p-1">
                      Gross Wt <br /> (Gms)
                    </div>
                    <div className="col_7_eia border-end border-black p-1">
                      Quantity <br /> PCS/PRS
                    </div>
                    ...
                    <div className="col_9_eia p-1">
                      Amount <br /> {result?.header?.CurrencyCode}
                    </div>
                  </div>
                  {metCatWiseData?.map((e, i) => {
                    return (
                      <div
                        className="d-flex fw-normal border border-top-0 border-black "
                        key={i}
                      >
                        <div className="col_1_eia fw-normal p-1">{i + 1}</div>
                        <div className="col_2_eia p-1">
                          {e?.Categoryname + " " + e?.MetalTypePurity}
                        </div>
                        <div className="col_3_eia p-1">
                ...
                <div className="col-6">
                  <div className="d-flex border-bottom border-black">
                    <div className="col-6 border-end border-black p-2">
                      <div className="">
                        <div className="pe-2 fw-semibold">
                          Invoice No. & Date : {result?.header?.EntryDate}
                        </div>
                        <div className="fw-normal">
                          {result?.header?.InvoiceNo}{" "}
                        </div>
                    ...
                    <div className="col-6 d-flex flex-column justify-content-between align-items-center p-2">
                      <p className="text-uppercase fw-semibold w-100">
                        Exporter's Ref.
                      </p>
                      <p className="text-uppercase fw-semibold"></p>
                    </div>
              ...
              {/* <div className={` w-100  h-100 mt-3`}>
                ...
                <div className=" border border-top-0 border-black p-1 text-start">
                  <p className="fw-normal">
                    <span className="fw-bold">AS ADD </span>&nbsp;&nbsp;&nbsp;{" "}
                    {metalValue?.QualityName} Branded {metalValue?.ShapeName} &{" "}
                    {silverValue?.QualityName} {silverValue?.ShapeName} Plain
                    Jewellery
                  </p>
                </div>
              </div> */}
              ...
              >
                ...
                <div
                  className={`${style.discriptionExport1}  border-black border-end`}
                >
                  <p className="text-center fw-bold">Description</p>
                </div>
                <div
                  className={`${style.designExport1}  border-black border-end`}
                >
                  <p className="text-center fw-bold">Design#</p>
                </div>
                <div
                  className={`${style.QuantityPcsExport1}  border-black border-end`}
                >
                  <p className="text-center fw-bold">Quantity Pcs</p>
                </div>
                <div
                  className={`${style.HSNCODEExport1}  border-black border-end`}
                >
                  <p className="text-center fw-bold">HSN CODE</p>
                </div>
                <div
                  className={`${style.GrossWtExport1}  border-black border-end`}
                >
                  <p className="text-center fw-bold">Gross Wt</p>
                </div>
                <div
              ...
              {data &&
                data.map((e, i) => {
                  return (
                    <div
                      className={`d-flex no_break border-black border-bottom border-start border-end ${style.rowExport1}`}
                      key={i}
                    >
                      <div
                        className={`${style.srNoExport1}  border-black border-end`}
              ...
              <div className="Pdata mt-3">
                ...
                {metalGoldPurityWise?.map((e, i) => {
                  return (
                    <div
                      ...
                      key={i}
                    >
                      <div className="col_t2_1_eia p-1 ">
                        {e?.MetalTypePurity}
                      </div>
                      <div className="col_t2_2_eia p-1 ">{e?.LossPer}%</div>
                      <div className="col_t2_3_eia p-1 ">
                        {e?.NetWt?.toFixed(3)}
                      </div>
                      {/* <div className='col_t2_4_eia p-1 '>{((e?.PureNetWt) - ((e?.LossWt * e?.Tunch)/99.5))?.toFixed(3)}</div> */}
                      <div className="col_t2_4_eia p-1 ">
                        {((e?.NetWt * e?.Tunch) / 99.5)?.toFixed(3)}
                      </div>
                      <div className="col_t2_5_eia p-1 ">
                        {e?.LossWt?.toFixed(3)}
                      </div>
                      <div className="col_t2_6_eia p-1 ">
                        {((e?.LossWt * e?.Tunch) / 99.5)?.toFixed(3)}
                      </div>
                      <div className="col_t2_7_eia p-1 ">
                        {(e?.NetWt + e?.LossWt)?.toFixed(3)}
                      </div>
                      <div className="col_t2_8_eia p-1">
                        {(((e?.NetWt + e?.LossWt) * e?.Tunch) / 99.5)?.toFixed(
                          3
                        )}
                      </div>
                    </div>
                ...
                {metalGoldPurityWise?.length > 0 && (
                  <div className="d-flex border border-black border-top-0 fw-semibold">
                    <div className="col_t2_1_eia p-1 fw-bold">
                      {metalGoldPurityWiseTotal?.ShapeName}
                    </div>
                    <div className="col_t2_2_eia p-1 "></div>
                    <div className="col_t2_3_eia p-1 fw-bold">
                      {metalGoldPurityWiseTotal?.NetWt?.toFixed(3)}
                    </div>
                    <div className="col_t2_4_eia p-1 fw-bold">
                      {(
                        metalGoldPurityWiseTotal?.PureNetWt -
                        metalGoldPurityWiseTotal?.pureLossWt
                      )?.toFixed(3)}
                    </div>
                    <div className="col_t2_5_eia p-1 fw-bold">
                      {metalGoldPurityWiseTotal?.LossWt?.toFixed(3)}
                    </div>
                    <div className="col_t2_6_eia p-1 fw-bold">
                      {metalGoldPurityWiseTotal?.pureLossWt?.toFixed(3)}
                    </div>
                    <div className="col_t2_7_eia p-1 fw-bold">
                      {(
                        metalGoldPurityWiseTotal?.LossWt +
                        metalGoldPurityWiseTotal?.NetWt
                      )?.toFixed(3)}
                    </div>
                    <div className="col_t2_8_eia p-1 fw-bold">
                      {metalGoldPurityWiseTotal?.PureNetWt?.toFixed(3)}
                    </div>
                  </div>
                )}

                {metalWithoutGoldPurityWise?.length > 0 && (
                  <div className="d-flex border border-black border-top-0 fw-semibold">
                    <div className="col_t2_1_eia p-1 fw-bold">
                      {metalWithoutGoldPurityWise[0]?.MetalType}
                    </div>
                    <div className="col_t2_2_eia p-1 "></div>
                    <div className="col_t2_3_eia p-1 "></div>
                    <div className="col_t2_4_eia p-1 "></div>
                ...
                {metalWithoutGoldPurityWise?.length > 0 && (
                  <div className="d-flex border border-black border-top-0 fw-semibold">
                    <div className="col_t2_1_eia p-1 fw-bold">
                      {metalWithoutGoldPurityWiseTotal?.ShapeName}
                    </div>
                    <div className="col_t2_2_eia p-1 "></div>
                    <div className="col_t2_3_eia p-1 fw-bold">
                      {metalWithoutGoldPurityWiseTotal?.NetWt?.toFixed(3)}
                    </div>
                    <div className="col_t2_4_eia p-1 fw-bold">
                      {(
                        metalWithoutGoldPurityWiseTotal?.PureNetWt -
                        metalWithoutGoldPurityWiseTotal?.pureLossWt
                      )?.toFixed(3)}
                    </div>
                    <div className="col_t2_5_eia p-1 fw-bold">
                      {metalWithoutGoldPurityWiseTotal?.LossWt?.toFixed(3)}
                    </div>
                    <div className="col_t2_6_eia p-1 fw-bold">
                      {metalWithoutGoldPurityWiseTotal?.pureLossWt?.toFixed(3)}
                    </div>
                    <div className="col_t2_7_eia p-1 fw-bold">
                      {(
                        metalWithoutGoldPurityWiseTotal?.NetWt +
                        metalWithoutGoldPurityWiseTotal?.LossWt
                      )?.toFixed(3)}
                    </div>
                    <div className="col_t2_8_eia p-1 fw-bold">
                      {metalWithoutGoldPurityWiseTotal?.PureNetWt?.toFixed(3)}
                    </div>
                  </div>
                )}
              ...
              <div className="ptotal border border-black" style={{ marginTop: '7px', padding: '5px' }}>
                <div style={{ display: "flex", gap: "10px" }}>
                  ...
                  <div style={{ display: "flex", gap: "5px" }}>
                    {" "}
                    <p>{result?.header?.CurrencyExchRate}</p>
                  </div>
                </div>
                <div style={{ display: "flex", gap: "10px" }}>
                  <p>F.O.B : </p>
          ...
          <div>4.000</div> */}
                ...
                <div className="d-flex border border-top-0  border-black fw-semibold">
                  <div className="col_t3_1_eia ps-1">
                    Convt. Rt {result?.header?.CurrencyExchRate} Value in Rs.{" "}
                  </div>
                  <div className="col_t3_2_eia fw-bold"></div>
                  <div className="col_t3_3_eia">
                    {/* <div>PCS</div>
                ...
                <div className="d-flex border border-top-0  border-black fw-semibold">
                  <div className="col_t3_1_eia ps-1">
                    Exchange Rate : {result?.header?.CurrencyExchRate}
                  </div>
                  <div className="col_t3_2_eia fw-bold"></div>
                  <div className="col_t3_3_eia"></div>
                  <div className="col_t3_4_eia"></div>
                  <div className="col_t3_5_eia"></div>
                  <div className="col_t3_6_eia"></div>
                </div>
              </div>
